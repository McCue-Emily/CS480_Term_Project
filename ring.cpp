...
Ring::Ring(int prec, const char* fname) { // prec is precision, or number of slices
...
void Ring::Render(GLint positionAttribLoc, GLint colorAttribLoc)
{
...
void Ring::init(int prec) {
    ...
    float outerRadius = 225;
    // 125 is the height of the texture png

    for (int k = 0; k < (outerRadius-innerRadius); k++) {
        float scale = (innerRadius + k) / outerRadius;
        for (int i = 0; i <= prec; i++) {
            for (int j = 0; j <= prec; j++) {
                // I want to draw a set of circles with different radii
                float y = ((float)cos(toRadians(180.f - i * 180.f / prec))) * scale;
                float x = (-(float)cos(toRadians(j * 360.f / prec)) * (float)abs(cos(asin(y)))) * scale;
